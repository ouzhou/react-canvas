...
/** \`JSON.stringify(undefined)\` 为 \`undefined\`，上游若直接传入会踩 \`.split\`；统一成字符串。 */
function normalizeCodeString(code: string | undefined | null): string {
  return code ?? "";
}
...
const getTokensCacheKey = (code: string, language: BundledLanguage) => {
  const safe = normalizeCodeString(code);
...
const createRawTokens = (code: string): TokenizedCode => ({
  bg: "transparent",
  fg: "inherit",
  tokens: normalizeCodeString(code)
    .split("\n")
    .map((line) =>
      line === ""
        ? []
...
export const highlightCode = (
  ...
  const safe = normalizeCodeString(code);
  const tokensCacheKey = getTokensCacheKey(safe, language);
  ...
  getHighlighter(language)
    ...
    .then((highlighter) => {
      ...
      const result = highlighter.codeToTokens(safe, {
        lang: langToUse,
        themes: {
          dark: "github-dark",
          light: "github-light",
...
export const CodeBlockContent = ({
  ...
  const safeCode = normalizeCodeString(code);
