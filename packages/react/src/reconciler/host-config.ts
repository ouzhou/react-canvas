...
function applyScrollViewHostProps(node: ScrollViewNode, p: Record<string, unknown>): void {
  if (p.horizontal === true) {
    node.horizontal = true;
  } else if (p.horizontal === false) {
    node.horizontal = false;
  }
  if (typeof p.scrollbarHoverVisible === "boolean") {
    node.scrollbarHoverVisible = p.scrollbarHoverVisible;
  }
}
...
export type PaintFrameRef = {
   ...
   * logical `width` / `height` exactly (see `canvasBackingStoreSize`) so `bw/lw === bh/lh`
   * and a single `scale(dpr)` is not anisotropic.
   */
  dpr: number;
  /** 画布根相机；与 `<Canvas camera={...}>` 同步，供绘制与命中。 */
  camera: ViewportCamera | null;
  /** `<Canvas>` 使用 core {@link Stage} 时设置；与 `surface` 同源，优先走 {@link Stage#requestLayoutPaint}。 */
  stage: Stage | null;
};

function requestLayoutPaintFromFrameRef(frameRef: PaintFrameRef): void {
  const root = frameRef.sceneRoot;
  if (!root) return;
  if (frameRef.stage) {
    frameRef.stage.requestLayoutPaint(root, frameRef.camera);
    return;
  }
  if (!frameRef.surface || !frameRef.canvasKit) return;
  queueLayoutPaintFrame(
    frameRef.surface,
    frameRef.canvasKit,
    root,
...
export function createCanvasHostConfig(
  ...
  return {
    ...
    resetAfterCommit: (_containerInfo: SceneContainer) => {
      const r = frameRef;
      if (!r.surface || !r.canvasKit) return;
      if (!paintRequesterRegistered) {
        paintFrameRequestFn = () => {
          requestLayoutPaintFromFrameRef(frameRef);
        };
      ...
      requestLayoutPaintFromFrameRef(r);
    },
    ...
    createInstance: (
      ...
      if (type === View) {
        ...
        const pv = props as Record<string, unknown>;
        if (typeof pv.onInteractionStateChange === "function") {
          node.onInteractionStateChange = pv.onInteractionStateChange as NonNullable<
            ViewNode["onInteractionStateChange"]
          >;
        }
        return node;
      }
      if (type === ScrollView) {
        ...
        node.setStyle(props.style ?? {});
        applyScrollViewHostProps(node, props as Record<string, unknown>);
        node.interactionHandlers = pickInteraction(props as Record<string, unknown>);
        return node;
      }
      if (type === Text) {
    ...
    commitUpdate: (
      ...
      if (instance.type === "Text") {
        ...
        applyScrollViewHostProps(instance as ScrollViewNode, nextProps as Record<string, unknown>);
      } else {
        updateViewNodeRef(
          instance,
          prevProps as Record<string, unknown>,
        ...
        if (instance.type === "View") {
          const np = nextProps as Record<string, unknown>;
          const fn = np.onInteractionStateChange;
          (instance as ViewNode).onInteractionStateChange =
            typeof fn === "function"
              ? (fn as NonNullable<ViewNode["onInteractionStateChange"]>)
              : undefined;
        }
