...
本文说明 `@react-canvas` 里 **Reconciler 场景节点**（`ViewNode` / `TextNode`）与 **Yoga 布局树** 的对应关系，以及 **测量 → 绘制** 时宽度如何传递。可与 [runtime-structure-constraints.md](./runtime-structure-constraints.md)（R-HOST-2 / R-HOST-3 / R-HOST-4）一起读。**换行、行高、`MeasureMode` 细节**见 [text-line-wrapping.md](./text-line-wrapping.md)。
...
**完整说明**（`MeasureMode`、`lineHeight`、`numberOfLines`、`\n` 等）：[text-line-wrapping.md](./text-line-wrapping.md)。
...
| 换行、行高、`MeasureMode`（文档）                     | [text-line-wrapping.md](./text-line-wrapping.md) |
