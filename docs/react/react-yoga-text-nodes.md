...
**`TextNode` 里再挂子 `TextNode`** 时：**不会**调用 `parent.yogaNode.insertChild(child.yogaNode)`，子节点 `yogaMounted = false`（见 `packages/core/src/scene/text-node.ts`）。
...
| 主题                                                  | 路径                                                   |
| ----------------------------------------------------- | ------------------------------------------------------ |
| `slots`、嵌套 `Text`、`yogaMounted`、`setMeasureFunc` | `packages/core/src/scene/text-node.ts`                 |
| `View` 对子节点 `yogaNode.insertChild`                | `packages/core/src/scene/view-node.ts`                 |
| `collectParagraphSpans`、span 展平                    | `packages/core/src/scene/text-node.ts`                 |
| `Paragraph` 构建、`layout`、`measureParagraphSpans`   | `packages/core/src/text/paragraph-build.ts`            |
| 换行、行高、`MeasureMode`（文档）                     | [text-line-wrapping.md](../core/text-line-wrapping.md) |
| `calculateLayoutRoot`、同步 `layout`                  | `packages/core/src/layout/layout.ts`                   |
| `drawParagraph`                                       | `packages/core/src/render/paint.ts`                    |
