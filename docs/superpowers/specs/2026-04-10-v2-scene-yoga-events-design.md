...
- 任何 **Skia / Canvas2D / 位图** 等 **由 SceneRuntime 承担的正式像素绘制**（DOM 调试叠加见 `docs/superpowers/specs/2026-04-10-v2-renderers-design.md`）。
- **DOM `PointerEvent`** 接入。
...
| 2026-04-10 | 实现：`core-v2` / `react-v2` / `apps/v2` 串联与测试                |
| 2026-04-10 | §7：与「V2 渲染子系统」规格对齐，区分 core 正式绘制与 DOM 调试叠加 |
